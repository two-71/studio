...
@import "@two-71/studio/styles.css";
```

That single import carries the `@source` directive (Tailwind never scans
node_modules on its own, so without it the studio's utility classes are
missing) plus the shadcn-style `data-checked:` / `data-open:` custom variants
the components are written against. Apps not generated by the shadcn CLI don't
have those variants, and without them switches render with no track colour and
dialogs with no enter/exit animation.

The studio reports its errors and confirmations through [sonner](https://sonner.emilkowal.ski)'s `toast()`, but deliberately doesn't mount a `<Toaster />` — that would double up for apps that already have one. Render one at your app root (`bunx shadcn@latest add sonner`, or `bun add sonner` and use its `<Toaster />` directly); without it every studio toast silently no-ops. See `apps/demo/components/ui/sonner.tsx` and `apps/demo/app/providers.tsx`.
